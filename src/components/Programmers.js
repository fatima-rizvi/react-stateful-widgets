...
export default function Programmers() {
  ...
  const [activeProgrammers, setActiveProgrammers] = useState(null)
  const getNameOfFeatured = () => {
    ...
    return programmers[activeProgrammers - 1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: activeProgrammers === null ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ setActiveProgrammers(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
